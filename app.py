...
# ============================================================
# INTERACTIVE NEURAL MASTERY MAP
# ============================================================

def build_neural_mastery_data(history, questions):
    """
    Build the node-and-edge structure used by the interactive Insights map.
    ...
    Skill brightness is based on smoothed mastery rather than raw accuracy
    alone. This prevents one correct answer from immediately displaying a
    skill as fully mastered.
    """
    stats = skill_stats(history)
    ...
    nodes = [
        {
            ...
            "confidence": min(100, round(len(history) / 30 * 100)),
    ...
    section_details = {
        "reading_writing": {
            "id": "section::reading_writing",
            "label": "Reading & Writing",
            "short_label": "Reading & Writing",
        },
        "math": {
            "id": "section::math",
            "label": "Math",
    ...
    # Add the two major section hubs.
    for section_key, detail in section_details.items():
        section_entries = [
            entry
            for entry in history
            if entry.get("section") == section_key
        ]

        section_attempts = len(section_entries)
        section_correct = sum(
            1 for entry in section_entries
            if entry.get("correct")
        )

        section_accuracy = (
            round(section_correct / section_attempts * 100)
            if section_attempts
            else None
        )

        # Bayesian-style smoothing toward 50 while evidence is limited.
        section_mastery = (
            round((section_correct + 3) / (section_attempts + 6) * 100)
            if section_attempts
            else 18
        )
        ...
        nodes.append(
            {
                ...
                "raw_accuracy": section_accuracy,
                "attempts": section_attempts,
                "correct": section_correct,
                "confidence": min(
                    100,
                    round(section_attempts / 20 * 100),
                ),
                "status": (
                    status_for_accuracy(section_accuracy)
        ...
        edges.append(
            {
                "source": "root",
                "target": detail["id"],
                "strength": section_mastery,
                "type": "section",
            }
        )

    # Use the question bank—not only practiced history—so the map represents
    # the student's complete SAT knowledge structure.
    skill_records = {}

    for question in questions:
        skill = str(question.get("skill", "")).strip()
        domain = str(question.get("domain", "Other")).strip()
        section = str(question.get("section", "")).strip()

        if not skill or section not in section_details:
            continue

        key = (section, domain, skill)

        if key not in skill_records:
            skill_records[key] = {
                "skill": skill,
                "domain": domain,
                "section": section,
    ...
    # Add domain hubs.
    for section, domain in domain_keys:
        domain_id = f"domain::{section}::{domain}"
        ...
        domain_attempts = sum(
            stats.get(skill, {}).get("n", 0)
            for skill in domain_skills
        )

        domain_correct = sum(
            stats.get(skill, {}).get("c", 0)
            for skill in domain_skills
        )

        domain_accuracy = (
            round(domain_correct / domain_attempts * 100)
            if domain_attempts
            else None
        )

        domain_mastery = (
            round((domain_correct + 2) / (domain_attempts + 4) * 100)
            if domain_attempts
            else 16
        )
        ...
        nodes.append(
            {
                ...
                "type": "domain",
                "section": section,
                "mastery": domain_mastery,
                "raw_accuracy": domain_accuracy,
                "attempts": domain_attempts,
                "correct": domain_correct,
                "confidence": min(
                    100,
                    round(domain_attempts / 12 * 100),
                ),
                "status": (
                    status_for_accuracy(domain_accuracy)
        ...
        edges.append(
            {
                "source": section_details[section]["id"],
                "target": domain_id,
    ...
    # Add individual skill nodes.
    for record in sorted(
        skill_records.values(),
        key=lambda item: (
            item["section"],
            item["domain"],
            item["skill"],
        ...
        skill_id = f"skill::{section}::{domain}::{skill}"
        domain_id = f"domain::{section}::{domain}"

        skill_stat = stats.get(skill, {})
        attempts = int(skill_stat.get("n", 0))
        correct = int(skill_stat.get("c", 0))
        ...
        # Smoothed mastery:
        # - Untested skills begin pale.
        # - Small samples remain conservative.
        # - Repeated strong performance creates richer colors.
        mastery = (
            round((correct + 2) / (attempts + 4) * 100)
            if attempts
            else 10
        )
        ...
        if attempts == 0:
            ...
            status = status_for_accuracy(raw_accuracy)
...
def render_neural_mastery_map(history, questions):
    """
    Render an animated canvas-based neural network.

    No external JavaScript library is required, so the visual continues
    working locally and during a hackathon demonstration.
    """
    graph_data = build_neural_mastery_data(history, questions)
            ...
            * {
            ...
            .network-shell {
                ...
                background:
                    radial-gradient(
                        circle at 22% 25%,
                        rgba(189, 112, 77, 0.12),
                        transparent 34%
                    ),
                    radial-gradient(
                        circle at 78% 68%,
                        rgba(112, 133, 103, 0.14),
                        transparent 38%
                    ),
                    linear-gradient(
                        145deg,
                ...
                box-shadow:
                    0 18px 44px rgba(76, 55, 36, 0.08),
                    0 3px 9px rgba(76, 55, 36, 0.04);
            }
            ...
            .network-title-block {
                max-width: 510px;
                padding: 13px 15px;
                border: 1px solid rgba(214, 201, 185, 0.86);
                border-radius: 14px;
                background: rgba(255, 252, 246, 0.86);
                box-shadow: 0 8px 24px rgba(76, 55, 36, 0.05);
            ...
            .tooltip {
                ...
                box-shadow:
                    0 17px 38px rgba(63, 44, 30, 0.15),
                    0 3px 9px rgba(63, 44, 30, 0.08);
                pointer-events: none;
                opacity: 0;
            ...
            .live-dot {
                ...
                border-radius: 50%;
                background: #b96547;
                box-shadow: 0 0 0 5px rgba(185, 101, 71, 0.12);
            ...
            @keyframes livePulse {
                0%,
                100% {
                    box-shadow: 0 0 0 4px rgba(185, 101, 71, 0.10);
                }

                50% {
                    box-shadow: 0 0 0 9px rgba(185, 101, 71, 0.02);
                }
            }
    ...
    <body>
        <div class="network-shell" id="networkShell">
            ...
            <div class="network-topbar">
                <div class="network-title-block">
                    ...
                    <div class="network-heading">
                        Your neural mastery network
                    </div>

                    <div class="network-description">
                        Each node represents a section, domain, or skill.
                        Richer color signals stronger mastery; larger nodes
            ...
            <div class="network-footer">
                ...
                <span>
                    Hover to inspect · Drag nodes to explore
                </span>
            </div>
        </div>
        ...
        <script>
            ...
            const shell = document.getElementById("networkShell");
            ...
            function paletteForNode(node) {
                const amount = clamp(node.mastery / 100, 0, 1);
                ...
                return [
                    interpolate(pale[0], rich[0], amount),
                    interpolate(pale[1], rich[1], amount),
                    interpolate(pale[2], rich[2], amount),
                ];
            }
            ...
            function nodeRadius(node) {
                ...
                if (node.type === "section") {
                    return 30 + Math.min(node.attempts, 20) * 0.18;
                }

                if (node.type === "domain") {
                    return 22 + Math.min(node.attempts, 16) * 0.16;
                }

                return 12 + Math.min(node.attempts, 10) * 0.75;
            }
            ...
            function shortLabel(label, maximum) {
                ...
                return label.slice(0, maximum - 1) + "…";
            }
            ...
            function initializePositions() {
                ...
                sectionNodes.forEach((node) => {
                    if (node.section === "reading_writing") {
                        node.x = width * 0.29;
                        node.y = height * 0.47;
                    } else {
                        node.x = width * 0.71;
                ...
                const domainGroups = {
                    reading_writing: graph.nodes.filter(
                        (node) =>
                            node.type === "domain" &&
                            node.section === "reading_writing"
                    ),
                    math: graph.nodes.filter(
                        (node) =>
                            node.type === "domain" &&
                            node.section === "math"
                    ),
                ...
                Object.entries(domainGroups).forEach(
                    ([section, domains]) => {
                        const centerX =
                        ...
                        const radiusX = Math.min(width * 0.19, 230);
                        const radiusY = Math.min(height * 0.31, 205);
                ...
                graph.nodes
                    ...
                    .forEach((node) => {
                        const key =
                            `domain::${node.section}::${node.domain}`;

                        if (!skillGroups[key]) {
                            skillGroups[key] = [];
                        }

                        skillGroups[key].push(node);
                    });

                Object.entries(skillGroups).forEach(
                    ([domainId, skills]) => {
                        const parent = nodeMap.get(domainId);
                ...
                graph.nodes.forEach((node, index) => {
                    if (!Number.isFinite(node.x)) {
                        node.x =
                            width * 0.5 +
                            Math.cos(index) * width * 0.25;
                    }

                    if (!Number.isFinite(node.y)) {
                        node.y =
                            height * 0.5 +
                            Math.sin(index) * height * 0.25;
                    }
            ...
            function applyForces() {
                for (const node of graph.nodes) {
                    ...
                    const pull = node.type === "skill" ? 0.006 : 0.009;

                    node.vx += (node.homeX - node.x) * pull;
                    node.vy += (node.homeY - node.y) * pull;
                }
                ...
                for (const node of graph.nodes) {
                    ...
                    const margin = nodeRadius(node) + 11;

                    node.x = clamp(node.x, margin, width - margin);
                    node.y = clamp(node.y, 92 + margin, height - margin);
                }
            }
            ...
            function drawBackgroundParticles() {
                ...
                for (let index = 0; index < count; index += 1) {
                    ...
                    const x =
                        (
                            seed * 79.17 +
                            Math.sin(animationTime * 0.0002 + seed) * 9
                        ) % width;

                    const y =
                        (
                            seed * 43.81 +
                            Math.cos(animationTime * 0.00015 + seed) * 8
                        ) % height;
                    ...
                    ctx.fillStyle = `rgba(151, 113, 84, ${alpha})`;
                    ctx.fill();
                }
            }
            ...
            function drawEdges() {
                graph.edges.forEach((edge, edgeIndex) => {
                    ...
                    const strength = clamp(edge.strength / 100, 0, 1);
                    const sourceColor = paletteForNode(source);
                    const targetColor = paletteForNode(target);
                    ...
                    gradient.addColorStop(
                        0,
                        rgbString(sourceColor, 0.13 + strength * 0.18)
                    );

                    gradient.addColorStop(
                        1,
                        rgbString(targetColor, 0.18 + strength * 0.28)
                    );
                    ...
                    const pulseProgress =
                        (
                            animationTime * 0.00016 +
                            edgeIndex * 0.127
                        ) % 1;

                    const oneMinus = 1 - pulseProgress;

                    const pulseX =
                        oneMinus * oneMinus * source.x +
                        2 *
                            oneMinus *
                            pulseProgress *
                            midX +
            ...
            function drawNode(node) {
                ...
                const color = paletteForNode(node);
                ...
                const highlight = [
                    Math.min(255, color[0] + 58),
                    Math.min(255, color[1] + 58),
                    Math.min(255, color[2] + 58),
                ];

                nodeGradient.addColorStop(
                    0,
                    rgbString(highlight, 1)
                );
                ...
                nodeGradient.addColorStop(
                    ...
                    rgbString(
                        [
                            Math.max(0, color[0] - 26),
                            Math.max(0, color[1] - 26),
                            Math.max(0, color[2] - 26),
                        ],
                        1
                    )
                );
                ...
                ctx.lineWidth = isHovered ? 2.3 : 1.15;
                ctx.stroke();

                const label =
                    node.type === "skill"
                        ? shortLabel(node.short_label, 16)
                        : shortLabel(node.short_label, 20);
                ...
                if (node.type === "skill") {
                    ctx.font =
                        `${isHovered ? 700 : 650} 9px Inter, sans-serif`;

                    ctx.fillStyle = "#46362d";
                    ...
                    ctx.font =
                        node.type === "root"
                            ...
                            : node.type === "section"
                                ? "700 10px Georgia, serif"
                                : "750 8px Inter, sans-serif";
            ...
            function showTooltip(node, x, y) {
                ...
                const accuracyText =
                    node.raw_accuracy === null
                        ? "Not tested"
                        : `${node.raw_accuracy}%`;

                const nodeTypeLabels = {
                    root: "Learning profile",
                    section: "SAT section",
                    domain: "SAT domain",
                    skill: "SAT skill",
                };
                ...
                const domainText =
                    node.domain
                        ? `<div class="tooltip-domain">${escapeHtml(node.domain)}</div>`
                        : "";

                tooltip.innerHTML = `
                    <div class="tooltip-type">
                        ${nodeTypeLabels[node.type] || "Knowledge node"}
                    </div>
            ...
            canvas.addEventListener("pointermove", (event) => {
                ...
                if (draggedNode) {
                    draggedNode.x = pointerX;
                    draggedNode.y = pointerY;
                    draggedNode.homeX = pointerX;
                    draggedNode.homeY = pointerY;
                    draggedNode.vx = 0;
                    draggedNode.vy = 0;
                ...
                hoveredNode = nodeAtPosition(pointerX, pointerY);
    ...
    # JavaScript uses braces extensively, so placeholder replacement is safer
    # than placing the whole document inside a Python f-string.
    neural_html = neural_html.replace(
        "__GRAPH_DATA__",
        graph_json,
    )

    # Replace the marker with the O(n²) collision-avoidance loop. Keeping it as
    # a placeholder prevents accidental formatting conflicts in the HTML string.
    neural_html = neural_html.replace(
        "#NODE_REPULSION#",
        """
                for (
                    let firstIndex = 0;
                    firstIndex < graph.nodes.length;
                    firstIndex += 1
                ) {
                    for (
        ...
        with strong_col:
            with st.container(border=True):
                ...
                if strong_list:
                    ...
                    empty_state(
                        "No skill has reached 80% yet — keep practicing "
                        "to build reliable strengths."
                    )
                else:
                    empty_state(
                        "Practice a few questions to reveal your strongest skills."
                    )

        section_header(
            "Cognitive network",
            "See how your SAT skills connect",
            (
                "SATSam maps every practiced skill into a living learning model. "
                "Color depth represents estimated mastery, while node size reflects "
                "how much evidence supports that estimate."
            ),
        )

        render_neural_mastery_map(
            history=history,
            questions=QUESTIONS,
        )

        st.caption(
            "Mastery is an experimental SATSam estimate based on accuracy and "
            "sample size. Hover over any node to inspect the supporting evidence."
        )
